...
#his has nothing to do with surrogate, just a thing i wanted to be added
df_long <- df_long %>% 
...
#here we mke new combinations of pairs - so the first pair column will specify the participant 1 and the second group id will specify the participant 2
...
p1 <- df %>% 
  ungroup() %>% 
  select(group_id, condition, starts_with(c("HR1","Resp1"))) %>% #we want all the participant 1's
...
#becuase we separated all the participant 1's and 2's, we can now combine them - and they will be mathced according to the new pairs we created with shuffled_pairs 
