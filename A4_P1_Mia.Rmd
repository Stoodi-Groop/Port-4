...
HR_model_surrogate <- lmer(HR_change_self ~ 0 + (HR_self + HR_other) : surrogatetype + (0 | participant) + (0 | group_id), subset(master_df, condition=="Synchronous"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))

summary(HR_model_Type)

resp_model_surrogate <- lmer(Resp_change_self ~ 0 + (Resp_self + Resp_other) : condition : surrogatetype + (0 + condition | participant) + (0 + condition | group_id),  subset(master_df, condition=="Synchronous"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
