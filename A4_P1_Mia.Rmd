...
df$condition <- relevel(df$condition, ref = "Synchronous")

# Model change as a function of own and other previous state 
HR_model0 <- lmer(HR_change_self ~ 0 + (HR_self + HR_other) : condition +  (0 + condition | participant) + (0 + condition | group_id),  subset(df, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))

resp_model0 <- lmer(Resp_change_self ~ 0 + (Resp_self + Resp_other) : condition +  (0 + condition | participant) + (0 + condition | group_id),  subset(df, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
...
HR_model_change_other <- lmer(HR_change_self ~ 0 + HR_change_other + (HR_self + HR_other) : condition +  (0 + condition | participant) + (0 + condition | group_id),  subset(df, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
Resp_model_change_other <- lmer(Resp_change_self ~ 0 +  (Resp_self + Resp_other) : condition +  Resp_change_other + (0 + condition | participant) + (0 + condition | group_id),  subset(df, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
...
df_sample <- df %>% group_by(participant, condition) %>%
  mutate(HR_self = sample(HR_self), 
         HR_other = sample(HR_other),
         Type = "Shuffled")

# Concatenate it to the original dataset (and remember to have a column telling you which is which)
df_shuffled <- bind_rows(df, df_sample)
...
shuffled_pairs <- pairs %>% 
  ...
  mutate(new_pair = sample(group_id),
         participant1 = paste0(group_id, 1),
         participant2 = paste0(new_pair, 2),
...
p1 <- df %>% 
  ungroup() %>% 
  select(group_id, condition, starts_with(c("HR1","Resp1"))) %>% 
  mutate(pair_id = paste0(condition, group_id)) %>% 
  select(starts_with(c("HR1","Resp1")), pair_id)

p2 <- df %>% 
  ungroup() %>% 
  select(group_id, condition, starts_with(c("HR2","Resp2"))) %>% 
  mutate(new_pair_id = paste0(condition, group_id)) %>% 
...
#adding to a main dataframe 
df_surrogate <- cbind(hr_self, HR_other = hr_other$HR_other)
...
df <-  df %>% 
  mutate(participant_id = participant,
         participant = str_extract(df$participant, "\\d$"))

new_df <- df_long %>%
  mutate(join_pair_id = paste0(condition, group_id))%>% 
...
data_surrogate <- new_df %>% 
  left_join(df_join)  

master_data <- data_surrogate %>% 
  select(HR_change_self, HR_change_other, HR_self, HR_other, 
         Resp_self, Resp_other, Resp_change_self, Resp_change_other,
