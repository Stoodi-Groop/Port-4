...
df <- df %>% 
  ...
  mutate(HR1_prev = lag(HR1_scaled),
         HR2_prev = lag(HR2_scaled),
         Resp1_prev = lag(Resp1_scaled),
         Resp2_prev = lag(Resp2_scaled))

# Generate a column for each: change in HR1, HR2, Resp1, Resp2

df <- df %>% 
  mutate(HR1_change = HR1_scaled - HR1_prev,
         HR2_change = HR2_scaled - HR2_prev,
         Resp1_change = Resp1_scaled - Resp1_prev,
         Resp2_change = Resp2_scaled - Resp2_prev)
# we say new value minus the previous, so if the change numbers are positive, it means that the hert rate has risen from before. 
?pivot_longer
