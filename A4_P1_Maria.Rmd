...
# Load the libraries
library(pacman)
p_load(RHRV, tidyverse, ggpubr, groupdata2) 
...
threshold=2.4 # Default value at 2.5 sds from the mean. But you can increase it, if you want.
...
#exclude id 27 (because it returns NaN)
df <- df %>% 
  filter(group_id != 27)
...
# Remove other bad data
threshold <-  2.4 #redundant beacuse it is already defined but hey
df <- df %>% 
  mutate(HR1 = removeOuts(df$HR1, threshold),
        ...
        HR1_new = scale(df$HR1),
        HR2_new = scale(df$HR2),
        Resp1_new = scale(df$Resp1),
        Resp2_new = scale(df$Resp2),
        time = ifelse(study == 3, time/60000, paste0(time))) 
...
#rename the _new columns to _scaled
colnames(df)[14:17] <- c("HR1_scaled", "HR2_scaled", "Resp1_scaled", "Resp2_scaled")
...
#make column where the previous/lagged HR and Resp is shown
df <- df %>% 
...
#make column for the change from the previous
df <- df %>% 
...
df_long <- cbind(hr_self, HR_other = hr_other$HR_other) #add column to whole dataframe
...
#write_csv(df_long, "longdf.csv")


df_long <- read_csv("longdf.csv")
```
...
#see levels for condition
levels(df_long$condition)

#relevel with synchronous as one
...
#small_subset <- subset(df_long, study=="3" & group_id == 37:39)
...
HR_model_change_other <- lmer(HR_change_self ~ 0 + (HR_self + HR_other) : condition + HR_change_other + (0 + condition | participant) + (0 + condition | group_id),  subset(df_long, study == "3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
#fails to converge?

summary(HR_model)
summary(HR_model_change_other)

#the same but for respiration 
Resp_model <- lmer(Resp_change_self ~ 0 + (Resp_self + Resp_other) : condition +  (0 + condition | participant) + (0 + condition | group_id),  subset(df_long, study == "3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))
...
HR_model_shuffled <- lmer(Resp_change_self ~ 0 + (Resp_self + Resp_other) : condition : Type + (0 + condition | participant) + (0 + condition | group_id),  subset(df_shuffled, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))

resp_model_shuffled <- lmer(Resp_change_self ~ 0 + (Resp_self + Resp_other) : condition : Type + (0 + condition | participant) + (0 + condition | group_id),  subset(df_shuffled, study=="3"), REML=F,control = lmerControl(optimizer = "nloptwrap", calc.derivs = FALSE))

summary(HR_model_shuffled)
summary(resp_model_shuffled)
